    ...
    for bs_idx in range(batch_size):
        ...
        if obj.shape[-1] == 1 and not nuscenes:
            ...
            topk_score, topk_ind = torch.topk(
                obj[topk_inds.view(-1)].squeeze(-1), K
            )  # torch.topk(topk_scores.view(-1), K)
        else:
            score = obj[batch_inds].permute(1, 0)
            topk_scores, topk_inds = torch.topk(score, min(K, score.shape[-1]))
            topk_score, topk_ind = torch.topk(
                topk_scores.view(-1), min(K, topk_scores.view(-1).shape[-1])
            )
...
def decode_bbox_from_voxels_nuscenes(
    batch_size,
    indices,
    obj,
    rot_cos,
    ...
    scores, inds, class_ids = _topk_1d(
        None, batch_size, batch_idx, obj, K=K, nuscenes=True
    )
    ...
    if not add_features is None:
        add_features = [
            gather_feat_idx(add_feature, inds, batch_size, batch_idx)
            for add_feature in add_features
        ]

    if not isinstance(feature_map_stride, int):
        feature_map_stride = gather_feat_idx(
            feature_map_stride.unsqueeze(-1), inds, batch_size, batch_idx
        )

    angle = torch.atan2(rot_sin, rot_cos)
    xs = (
        spatial_indices[:, :, -1:] + center[:, :, 0:1]
    ) * feature_map_stride * voxel_size[0] + point_cloud_range[0]
    ys = (
        spatial_indices[:, :, -2:-1] + center[:, :, 1:2]
    ...
    if not add_features is None:
        add_features = [
            add_feature.view(batch_size, K, add_feature.shape[-1])
            for add_feature in add_features
        ]
    ...
    for k in range(batch_size):
        ...
        cur_labels = final_class_ids[k, cur_mask]
        cur_add_features = (
            [add_feature[k, cur_mask] for add_feature in add_features]
            if not add_features is None
            else None
        )
...
def decode_bbox_from_pred_dicts(
    ...
    ys, xs = torch.meshgrid(
        ...
        indexing="ij",  # or "xy", depending on your needs
    )

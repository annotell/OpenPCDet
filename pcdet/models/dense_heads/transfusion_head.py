...
class TransFusionHead(nn.Module):
    ...
    def __init__(
        ...
        self.dataset_name = self.model_cfg.TARGET_ASSIGNER_CONFIG.get(
            "DATASET", "nuScenes"
        )
        ...
        layers.append(
            BasicBlock2D(
                hidden_channel, hidden_channel, kernel_size=3, padding=1, bias=bias
            )
        )
        layers.append(
            nn.Conv2d(
        ...
        self.prediction_head = SeparateHead_Transfusion(
            hidden_channel, 64, 1, heads, use_bias=bias
        )
    ...
    def create_2D_grid(self, x_size, y_size):
        ...
        batch_x, batch_y = torch.meshgrid(
            *[torch.linspace(it[0], it[1], it[2]) for it in meshgrid],
            indexing="ij",  # or "xy", depending on the intended indexing
        )
        batch_x = batch_x + 0.5
        batch_y = batch_y + 0.5
    ...
    def get_targets_single(self, gt_bboxes_3d, gt_labels_3d, preds_dict):
        ...
        pos_inds = (
            torch.nonzero(assigned_gt_inds > 0, as_tuple=False).squeeze(-1).unique()
        )
        neg_inds = (
            torch.nonzero(assigned_gt_inds == 0, as_tuple=False).squeeze(-1).unique()
        )
        ...
        for idx in range(len(gt_bboxes_3d)):
            ...
            if width > 0 and length > 0:
                radius = centernet_utils.gaussian_radius(
                    length.view(-1),
                    width.view(-1),
                    target_assigner_cfg.GAUSSIAN_OVERLAP,
                )[0]
                radius = max(target_assigner_cfg.MIN_RADIUS, int(radius))
                x, y = gt_bboxes_3d[idx][0], gt_bboxes_3d[idx][1]

                coor_x = (
                    (x - self.point_cloud_range[0])
                    / self.voxel_size[0]
                    / self.feature_map_stride
                )
                coor_y = (
                    (y - self.point_cloud_range[1])
                    / self.voxel_size[1]
                    / self.feature_map_stride
                )
    ...
    def loss(self, gt_bboxes_3d, gt_labels_3d, pred_dicts, **kwargs):
        ...
        loss_heatmap = (
            ...
            / max(heatmap.eq(1).float().sum().item(), 1)
        )
        ...
        loss_cls = self.loss_cls(cls_score, one_hot_targets, label_weights).sum() / max(
            num_pos, 1
        )

        preds = torch.cat(
            [
                pred_dicts[head_name]
                for head_name in self.model_cfg.SEPARATE_HEAD_CFG.HEAD_ORDER
            ],
            dim=1,
        ).permute(0, 2, 1)
    ...
    def decode_bbox(self, heatmap, rot, dim, center, height, vel, filter=False):
        ...
        center[:, 0, :] = (
            center[:, 0, :] * self.feature_map_stride * self.voxel_size[0]
    ...
    def get_bboxes(self, preds_dicts):
        ...
        one_hot = F.one_hot(self.query_labels, num_classes=self.num_classes).permute(
            0, 2, 1
        )

...
class ProposalTargetLayerMPPNet(ProposalTargetLayer):
                ...
                for idx in range(0, batch_dict["num_frames"]):
                    if idx == cur_frame_idx:
                        batch_trajectory_rois_list.append(
                            cur_trajectory_rois[
                                cur_frame_idx : cur_frame_idx + 1, sampled_inds
                            ]
                        )
                        continue
    ...
    def random_aug_box3d(self, box3d):
        ...
        if self.roi_sampler_cfg.REG_AUG_METHOD == "single":
            ...
            pos_shift = (
                (torch.rand(3, device=box3d.device) - 0.5) / 0.5
            ) * range_config[idx][0]
            hwl_scale = (
                (torch.rand(3, device=box3d.device) - 0.5) / 0.5
            ) * range_config[idx][1] + 1.0
            angle_rot = (
                (torch.rand(1, device=box3d.device) - 0.5) / 0.5
            ) * range_config[idx][2]
...
class MPPNetHead(RoIHeadTemplate):
    def __init__(self, model_cfg, num_class=1, **kwargs):
        ...
        self.up_dimension_geometry = MLP(
            input_dim=29,
            hidden_dim=64,
            output_dim=hidden_dim // num_radius,
            num_layers=3,
        )
                    ...
                    torch.cat(
                        [
                            ...
                            for i in torch.meshgrid(
                                torch.arange(
                                    self.grid_size, dtype=torch.float32, device="cuda"
                                ),
                                torch.arange(
                                    self.grid_size, dtype=torch.float32, device="cuda"
                                ),
                                ...
                                indexing="ij",  # or "xy", depending on your intended indexing
                            )
                        ],
                        dim=1,
                    )
    ...
    def roi_grid_pool(
        ...
        if batch_cnt is None:
            xyz_batch_cnt = (
                torch.tensor([num_points] * num_rois * batch_size).cuda().int()
            )
        else:
            xyz_batch_cnt = torch.tensor(batch_cnt).cuda().int()

        new_xyz_batch_cnt = (
            torch.tensor([num_proxy_points] * num_rois * batch_size).cuda().int()
        )
        ...
        features = pooled_features.view(
            ...
            num_frames * self.num_proxy_points,
    ...
    def crop_current_frame_points(
        ...
        for bs_idx in range(batch_size):
            ...
            cur_points = batch_dict["points"][(batch_dict["points"][:, 0] == bs_idx)][
                :, 1:
            ]
            dis = torch.norm(
                (
    ...
    def crop_previous_frame_points(
        ...
        for bs_idx in range(batch_size):
            ...
            for idx in range(1, trajectory_rois.shape[1]):
                ...
                cur_radiis = (
                    torch.sqrt(
                        (cur_batch_boxes[:, 3] / 2) ** 2
                        + (cur_batch_boxes[:, 4] / 2) ** 2
                    )
                    * 1.1
                )
                ...
                point_mask = (dis <= cur_radiis.unsqueeze(-1)).view(
                    trajectory_rois.shape[2], -1
                )
                ...
                for roi_box_idx in range(0, num_rois):
                    ...
                    if cur_roi_points.shape[0] > self.num_lidar_points:
                        ...
                        cur_roi_points_sample = cur_roi_points.new_zeros(
                            self.num_lidar_points, 6
                        )
                        ...
                        cur_roi_points_sample = torch.cat(
                            [cur_roi_points, add_zeros], dim=0
                        )
    ...
    def get_proposal_aware_motion_feature(
        ...
        padding_zero = torch.zeros(
            [proxy_point.shape[0], proxy_point.shape[1], 2]
        ).cuda()
        ...
        corner_points, _ = self.get_corner_points_of_roi(
            trajectory_rois[:, 0, :, :].contiguous()
        )
        corner_points = corner_points.view(
            batch_size, num_rois, -1, corner_points.shape[-1]
        )
        corner_points = corner_points.view(batch_size * num_rois, -1)
        trajectory_roi_center = trajectory_rois[:, 0, :, :].reshape(
        ...
        proposal_aware_feat = torch.cat(
            [proposal_aware_feat, proxy_point_time_padding], -1
        )
    ...
    def forward(self, batch_dict):
        ...
        if self.training:
            ...
            targets_dict["trajectory_rois"][
                :, batch_dict["cur_frame_idx"], :, :
            ] = batch_dict["rois"]
            ...
            if self.avg_stage1_score:
                ...
                if self.model_cfg.get("IOU_WEIGHT", None):
                    ...
                    for bs_idx in range(batch_size):
                        ...
                        cls_preds = torch.cat(
                            [batch_cls_preds_car, batch_cls_preds_pedcyc], 1
                        )
                        box_preds = torch.cat(
                            [
    ...
    def get_box_reg_layer_loss(self, forward_ret_dict):
        ...
        if loss_cfgs.REG_LOSS == "smooth-l1":
            ...
            rois_anchor = (
                roi_boxes3d.clone().detach()[:, :, :7].contiguous().view(-1, code_size)
            )
            rois_anchor[:, 0:3] = 0
            rois_anchor[:, 6] = 0
            ...
            if self.model_cfg.USE_AUX_LOSS:
                ...
                seqbox_loss_reg = (
                    seqbox_loss_reg.view(rcnn_batch_size, -1)
                    * fg_mask.unsqueeze(dim=-1).float()
                ).sum() / max(fg_sum, 1)
                seqbox_loss_reg = (
                    seqbox_loss_reg
            ...
            if loss_cfgs.CORNER_LOSS_REGULARIZATION and fg_sum > 0:
                ...
                fg_roi_boxes3d = (
                    roi_boxes3d[:, :, :7].contiguous().view(-1, code_size)[fg_mask]
                )
    ...
    def generate_predicted_boxes(
        self, batch_size, rois, cls_preds=None, box_preds=None
    ):
        """
        Args:
            batch_size:

...
class MPPNetHeadE2E(RoIHeadTemplate):
                ...
                self.grid_index = torch.cat(
                    [
                        ...
                        for i in torch.meshgrid(
                            ...
                            indexing="ij",  # Use "ij" or "xy" based on your intended behavior
                        )
                    ],
                    dim=1,
                ).float()
    ...
    def get_dense_grid_points(rois, batch_size_rcnn, grid_size):
        ...
        roi_grid_points = torch.div(
            (dense_idx + 0.5), grid_size
        ) * local_roi_size.unsqueeze(dim=1) - (local_roi_size.unsqueeze(dim=1) / 2)
    ...
    def roi_grid_pool(
        ...
        if batch_cnt is None:
            xyz_batch_cnt = (
                torch.tensor([num_points] * rois.shape[2] * batch_size).cuda().int()
            )
        else:
            xyz_batch_cnt = torch.tensor(batch_cnt).cuda().int()
        new_xyz = torch.cat(
            [i[0] for i in global_roi_proxy_points.chunk(rois.shape[2], 0)], 0
        )
        new_xyz_batch_cnt = (
            torch.tensor([self.num_proxy_points] * rois.shape[2] * batch_size)
            .cuda()
            .int()
        )
    ...
    def get_proposal_aware_motion_feature(
        ...
        corner_add_center_points = torch.cat(
            [
                corner_points,
                trajectory_rois[:, 0, :, :].reshape(batch_size * num_rois, -1)[:, :3],
            ],
            dim=-1,
        )

        pos_fea = proxy_point[:, :, :3].repeat(
            1, 1, 9
    ...
    def transform_prebox_to_current_vel(self, pred_boxes3d, pose_pre, pose_cur):
        ...
        moved_bboxes_global[:, :2] = (
            moved_bboxes_global[:, :2] - 0.1 * vels_global[:, :2]
        )
        ...
        moved_bboxes_pre2cur = np.dot(
            expand_moved_bboxes_global, np.linalg.inv(pose_cur.T)
        )[:, :3]
    ...
    def generate_trajectory(self, cur_batch_boxes, proposals_list, batch_dict):
        ...
        matching_table = (
            trajectory_rois.new_ones(
                [trajectory_rois.shape[1], trajectory_rois.shape[2]]
            )
            * -1
        ).long()
    ...
    def forward(self, batch_dict):
        ...
        if batch_dict["sample_idx"][0] >= 1:
            ...
            src_repeat = src_geometry_feature[
                :, None, : self.num_proxy_points, :
            ].repeat([1, trajectory_rois.shape[1], 1, 1])
            ...
            src_geometry_feature = src_geometry_feature.repeat(
                [1, trajectory_rois.shape[1], 1]
            )

        batch_dict["geometory_feature_memory"] = src_geometry_feature[
            :, : self.num_proxy_points
        ]

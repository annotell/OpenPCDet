...
class Cube3D:
    def __init__(self, scale: np.ndarray, rotation: np.ndarray, coordinates: np.ndarray):
        self.rotation = np.asarray(rotation)  # Quaternions with order: (x, y, z, w)
    ...
    def corners(self, wlh_factor: float = 1.0) -> np.ndarray:
        ...
        corners[0, :] = w / 2 * np.array([1, -1, -1, 1, 1, -1, -1, 1])  # x_corners
        corners[1, :] = l / 2 * np.array([1, 1, 1, 1, -1, -1, -1, -1])  # y_corners
        corners[2, :] = h / 2 * np.array([1, 1, -1, -1, 1, 1, -1, -1])  # z_corners

        # Rotate & Translate
        corners = self.rotation_matrix @ corners + self.coordinates.reshape((3, -1))
    ...
    def calculate_intersection(self, shape_2):
        ...
        num_samples = 10000
        points_in_box_1 = np.random.uniform(-0.5, 0.5, size=(3, num_samples)) * self.get_scale().reshape(3, -1)

        # transformation from box_1 reference system to box_2 reference system
        point_in_box_2 = R2_inv @ (R1 @ points_in_box_1 + self.calculate_relative_position(shape_2=shape_2))
    ...
    def calculate_iou(self, shape_2):
        ...
        if intersection_volume == 0:
            return 0
        union_volume = self.calculate_volume() + shape_2.calculate_volume() - intersection_volume
    ...
    def transform_points_to_box_coordinate_system(self, points: np.ndarray):
        ...
        """
        points_box = self.inverse_rotation_matrix @ (points.T - self.coordinates.reshape((3, -1)))
        return points_box.T
...
class ObjectCentricTrainDataset(DatasetTemplate):
    ...
    def random_box(cuboid):
        ...
        delta_coordinates = np.random.uniform(coordinates - scale / 2, coordinates + scale / 2)
        # add noise to quaternion rotation
        delta_rotation = np.random.normal(rotation, 0.1)
        noise_cuboid = Cube3D(coordinates=delta_coordinates, rotation=delta_rotation, scale=scale)

        return noise_cuboid

    def get_noise_pc_and_box(self, cuboid, points):
        # select a random subset of points costing of a percentage between 10% and 95% of the total number of points
        percentage = np.random.uniform(0.1, 0.95)
        cuboid_points = self.get_cuboid_points(cuboid, points)
        n_points = int(percentage * cuboid_points.shape[0])
        # select random points
        random_idxs = np.random.choice(cuboid_points.shape[0], n_points, replace=False)
        # select points
        cuboid_points = cuboid_points[random_idxs, :]
        # # add noise to points
        cuboid_points[:, :3] = cuboid_points[:, :3] + np.random.normal(0, 0.05, cuboid_points[:, :3] .shape)

        noise_cuboid = self.random_box(cuboid)
        points[:, :3] = noise_cuboid.transform_points_to_box_coordinate_system(points[:, :3])
        inv_quat = Rotation.from_matrix(noise_cuboid.inverse_rotation_matrix).as_quat()
        inv_coord = -(noise_cuboid.inverse_rotation_matrix @ noise_cuboid.coordinates).T
    ...
    def generate_sample(self, path):
        ...
        cuboid = Cube3D(df.Cube3D_scale[0], df.Cube3D_rotation[0], df.Cube3D_coordinates[0])
        cuboid_local = Cube3D(df.Cube3D_scale[0], np.array([0, 0, 0, 1]), np.array([0, 0, 0]))
        points[:, :3] = cuboid.transform_points_to_box_coordinate_system(points[:, :3])
    ...
    def __getitem__(self, index):
        ...
        input_dict.update({"gt_names": annotation["name"], "gt_boxes": annotation["gt_boxes_lidar"]})

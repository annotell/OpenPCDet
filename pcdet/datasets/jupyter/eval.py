...
def clean_data(gt_anno, dt_anno, current_class, difficulty):
    CLASS_NAMES = ['car', 'heavyduty', 'pedestrian', 'twowheeler']
    MIN_HEIGHT = [40, 25, 25]
    MAX_OCCLUSION = [0, 1, 2]
...
def get_official_eval_result(gt_annos, dt_annos, current_classes, PR_detail_dict=None):
    min_overlaps = np.array([[[0.7, 0.7, 0.5, 0.5],
                              [0.7, 0.7, 0.5, 0.5],
                              [0.7, 0.7, 0.5, 0.5]]])

    cls_to_index = {"Vehicle": 0, "School_bus": 1, "Vehicle_door": 2, "Ambulance": 3, "Fire_truck": 4,
                    'Four wheel police vehicle': 5}

    class_to_name = {v: k for k, v in cls_to_index.items()}
    ...
    result = ''
    mAPbev, mAP3d, mAPbev_R40, mAP3d_R40 = do_eval(gt_annos, dt_annos, current_classes, min_overlaps,
    ...
    for j, curcls in enumerate(current_classes):
        ...
        for i in range(min_overlaps.shape[0]):
            ...
            result += print_str(
                (f"{class_to_name[curcls]} "
                 "AP_R40@{:.2f}:".format(*min_overlaps[i, :, j])))

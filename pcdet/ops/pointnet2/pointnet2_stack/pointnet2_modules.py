...
def build_local_aggregation_module(input_channels, config):
    ...
    if local_aggregation_name == "StackSAModuleMSG":
        ...
        cur_layer = VectorPoolAggregationModuleMSG(
            input_channels=input_channels, config=config
        )
...
class VectorPoolLocalInterpolateModule(nn.Module):
    def __init__(
        ...
        self.num_total_grids = (
            self.num_voxels[0] * self.num_voxels[1] * self.num_voxels[2]
        )
...
class VectorPoolAggregationModule(nn.Module):
    def __init__(
        ...
        self.separate_local_aggregation_layer = nn.Sequential(
            nn.Conv1d(
                num_c_in, num_c_out, kernel_size=1, groups=self.total_voxels, bias=False
            ),
    ...
    def vector_pool_with_voxel_query(
        ...
        new_features = torch.cat((new_local_xyz, new_features), dim=-1).view(
            num_new_pts, -1
        )
    ...
    def get_dense_voxels_by_center(point_centers, max_neighbour_distance, num_voxels):
        ...
        x_offset, y_offset, z_offset = torch.meshgrid(
            ...
            y_grids,
            z_grids,
            indexing="ij",  # or "xy" based on your intended indexing
    ...
    def forward(
        self, xyz, xyz_batch_cnt, new_xyz, new_xyz_batch_cnt, features, **kwargs
    ):
        """
        :param xyz: (N1 + N2 ..., 3) tensor of the xyz coordinates of the features
        :param xyz_batch_cnt: (batch_size), [N1, N2, ...]
